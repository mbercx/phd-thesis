...
settings used to obtain all of the results presented in this thesis, organized 
...
\dft{plane wave basis}{} set and offering several \dft{PAW}{} data sets for 
...
data sets for the different elements present in the crystal structure. \gls{VASP} 
...
The final lattice vectors and atom positions of the unit cells are written to 
...
and the components of the magnetization density should be provided for each atom, 
...
as valence electrons within the \dft{PAW}{} framework. Hence, 
...
(``\vasp{CSHIFT}''). After calculating the real part, however, \gls{VASP} also 
...
$D_v(\epsilon)$ and conduction $D_c(\epsilon)$ band as input, as well as 
...
the valence band $\phi = \epsilon_0 - \epsilon_v$.
...
($D_v(\epsilon)$ and $D_c(\epsilon)$) states as input, as well as 
the vacuum level. We calculate the density of states and vacuum level of 
...
in case the vacuum layer is sufficiently thick. The work function $\phi$ of 
the surface is then calculated by comparing the vacuum level with the top of 
the Fermi level $\phi = \epsilon_0 - \epsilon_v$. 
...
\textit{A Poisson process is a renewal process in which the inter arrival 
...
\end{equation}
Within our model, an electron with energy $\epsilon$ can excite plasmons up to 
energy $\epsilon - \epsilon_F$, and the rate of exciting a volume plasmon with 
energy $E_{vp}$ is proportional to the energy loss function $L(E)$:
\begin{equation} 
\lambda_{vp}(E_{vp}) = 
\begin{cases} 
c_{vp} \cdot L(E_{vp}) &\text{  for } 0 < E_{vp} \leq \epsilon - \epsilon_F \\
0 &\text{  for } E_{vp} \leq 0 \text{ or } E_{vp} > \epsilon - \epsilon_F.
\end{cases}
\end{equation} 
...
\begin{equation}
P_{vp}(\epsilon, E_{vp}) = \text{Pr}\{T_{vp}(E_{vp}) = \text{min}(T_{vp}(E)\} = \frac{\lambda_{vp}(E_{vp})}{\int_0^{\epsilon - \epsilon_F} \lambda_{vp}(E) dE}.
\end{equation}
Note that this distribution is properly normalized to 1:
\begin{eqnarray*}
\int_0^\infty P_{vp}(\epsilon, E_{vp}) dE_{vp} &=& \int_0^\infty P_{vp}(\epsilon, E_{vp}) dE_{vp}  \\
&=& \frac{\int_0^\infty \lambda_{vp}(E_{vp}) dE_{vp}}{\int_0^{\epsilon - \epsilon_F} \lambda_{vp}(E) dE} \\
&=& \frac{c_{vp} \int_0^{\epsilon - \epsilon_F} L(E_{vp}) dE_{vp}}{c_{vp} \int_0^{\epsilon - \epsilon_F} L(E) dE} = 1 \\
\end{eqnarray*}
...
$\int_0^{\epsilon - \epsilon_F} \lambda_{vp}(E) dE$, the probability of 
any volume plasmon being excited in the interval $t_e$ is:
\begin{equation}
\text{Pr}\{\text{min}(T_{vp}(E)) < t_e \} = 1 - e^{-(\int_0^{\epsilon - \epsilon_F} \lambda_{vp}(E) dE) t_e}.
\end{equation}
...
\begin{eqnarray}
P_{vp}(\epsilon, E_{vp}) dE_{vp} &=& \frac{\lambda_{vp}(E_{vp})dE_{vp}}{\int_0^{\epsilon - \epsilon_F} \lambda_{vp}(E) dE }\left[1-e^{-\left(\int_0^{\epsilon - \epsilon_F} \lambda_{vp}(E_{vp}) dE\right) t_e }\right] \nonumber \\
&=& \frac{c_{vp} L(E_{vp})dE_{vp}}{\int_0^{\epsilon - \epsilon_F} c_{vp} L(E) dE }\left[1-e^{-\left(\int_0^{\epsilon - \epsilon_F} c_{vp} L(E) dE\right) t_e }\right] \nonumber \\
...
Consider the distribution of excited electrons $N_i(\epsilon)$. The 
probability that an excited electron at energy $\epsilon$ induces a volume 
plasmon of energy $E_{vp} < \epsilon - \epsilon_F$ is given by Eq.~(\ref{appendix:eq-vp_prob}). The fraction of electrons which excite a volume plasmon is then equal to: 
\begin{equation} 
    N_{vp}^{-}(\epsilon) = \int_0^\infty N_i(\epsilon) 
P_{vp}(\epsilon, E_{vp}) dE_{vp}. 
\end{equation} 
These are subtracted from the excited electron density. However, these 
...
\begin{equation} 
    N_{vp}^{+}(\epsilon) = \int_0^\infty N_i(\epsilon + E_{vp}) 
P_{vp}(\epsilon, E_{vp}) dE_{vp}. 
\end{equation} 
...
\begin{equation} 
    D_{vp} (E_{vp})= \int_0^\infty N_i(\epsilon) P_{vp}(\epsilon, 
E_{vp}) d\epsilon 
\end{equation} 
When these plasmons decay, they release an energy equal to $E_{vp}$. In other 
words, this distribution should be convoluted with the density of the occupied 
states $D_v(\epsilon)$ in order to obtain the density of excited electrons 
due to volume plasmon decay: 
\begin{eqnarray} 
    N_{vp}^{d}(\epsilon) &=& \frac{1}{n_{vp}}(D_v * D_{vp}) (\epsilon) 
\\ 
    &=& \frac{1}{n_{vp}} \int_{\Delta\epsilon_v} D_v(\epsilon_1) 
D_{vp}(\epsilon - \epsilon_1) d\epsilon_1 \\ 
    &=& \frac{1}{n_{vp}} \int_{\Delta\epsilon_v} D_v(\epsilon_1)  
    \int_0^\infty N_i(\epsilon_2) P_{vp}(\epsilon_2, \epsilon - 
...
\begin{equation} 
    n_{vp} = \int_0^\infty N_{vp}^{-}(\epsilon) d\epsilon 
\end{equation} 
...
(\epsilon_2) \delta(\epsilon - \epsilon_1 - \epsilon_2 + 
\epsilon_0 - E_I) d\epsilon_1 d\epsilon_2. \\ 
&= \int_{\Delta\epsilon_v} D_v (\epsilon_1) D_v (\epsilon + 
\epsilon_0 - E_I - \epsilon_1) d\epsilon_1 \\  
&= (D_v * D_v) (\epsilon + \epsilon_0 - E_I) 
\end{aligned} 
\end{equation} 
which can also be written as: 
\begin{equation} 
T\left[\frac{\epsilon + \epsilon_0 - E_I}{2}\right] = D_v(\epsilon) * 
D_v(\epsilon + \epsilon_0 - E_I) 
\end{equation} 
 
Here, $D_E(\epsilon) = D_v(\epsilon + \epsilon_0 - E_I)$ 
...
$\epsilon = 0$ in $D_E(\epsilon)$. We then obtain $D_v(\epsilon_0 - 
...
understanding this is that an electron at energy level $\epsilon_0 - E_I + 
\epsilon$ releases an energy of $\epsilon$ when it neutralizes the 
...
incoming ion, i.e. $\epsilon$ in $D_E(\epsilon)$. The leftover 
distribution of energies passed to other valence electrons through an Auger 
process is: 
\begin{equation} 
    D_{aug}(\epsilon) = D_E(\epsilon) (1 - P_{sp}(\epsilon)) 
\end{equation} 
We substitute this distribution in the expression for the Auger transform: 
\begin{equation} 
\begin{aligned} 
T'\left[\frac{\epsilon + \epsilon_0 - E_I}{2}\right] &= D_v(\epsilon) 
* D_{aug}(\epsilon) \\ &= D_v(\epsilon) * (D_v(\epsilon + 
\epsilon_0 - E_I) (1 - P_{sp}(\epsilon))) 
\end{aligned} 
\end{equation} 
