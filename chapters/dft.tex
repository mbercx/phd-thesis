...
\begin{figure}[ht]  
\centering 
\input{\figurepath/dft/scf_cycle}
...
\rho\right|$. GGA's generally produce better results than the LDA, but 
...
non-empirical GGA functional that is very popular in solid state physics is the 
...
\end{equation} 
where $E_{DFT}$ is the energy obtained from the uncorrected DFT result, and $\rho^\sigma_{mm'}$ is the spin orbital density matrix of the orbitals to which we 
...
\begin{figure}[ht] 
\centering
\input{\figurepath/dft/transition} 
