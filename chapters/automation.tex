...
writing papers. Moreover, some properties can not be feasibly calculated in 
...
tshe \code{Firetask}. Each \code{Firework} of the workflow consists of one or 
more \code{Firetask}s, which are executed in the order that they have in the 
list specified when initializing the \code{Firework}. While the workflow is 
...
\texttt{KPAR} k-points in parallel, i.e. it divides the available cores and 
...
of cores is divided among \texttt{NPAR = 4} groups, resulting in 
...
this, consider the following example: let's say you want to do a geometry 
...
numbers. That said, choosing \texttt{KPAR = 1} would be a mistake, since the 
parallelization will more than make up for the time lost due to inactive 
cores. So, let's say you choose \texttt{KPAR = 2}. In this case, \texttt{VASP} 
divides the cores over two groups, and hence runs two k-points simultaneously. 
...
However, I've noticed that for the \texttt{RMM-DIIS} algorithm, it seems that 
the optimal value of \texttt{NPAR/NCORE} is different. More testing is 
required\footnote{By the time you are reading this, the algorithm might have 
already changed, since it's the first thing I plan to do after submitting my 
thesis.} to optimize this problem further. Moreover, it's probably better to 
...
When running large numbers of calculations using workflows, you're bound to 
...
the quotas workflow~\ref{automation:sec-surface}. 
...
energy of a whole range of configurations for e.g. and alloy system. 
...
number of steps required here is rather limited, it's an ideal problem to 
...
also sets the \vasp{CSHIFT} parameter to 0.01, which should be low enough so 
that \texttt{VASP} does not overwrite the original imaginary part of the 
dielectric tensor with the one obtained from the Kramers-Kronig relation. This 
...
configuration~\ref{batteries:sec-structure}. 
Figure~\ref{automation:fig-configurations} shows the configuration workflow, 
which at first only consists a single \code{Firework} that generates all 
...
passed on to a small \code{Firework} that sets up the a transition workflow 
...
reaction energy is smaller than a certain tolerance (e.g. zero to only 
consider thermodynamically favorable dimers), the kinetic barrier is 
...
several possible terminations for each set of miller indices. It's good 
...
because of the ionic nature of the bonds in the structure, there is a dipole 
...
several elements that to consider before finalizing the unit cell for the 
...
Section~\ref{quotas:sec-} requires the density of states and the vacuum level. 
