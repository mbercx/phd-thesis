...
\caption{\label{slme:fig-solar} Solar Spectra for the extraterrestrial \gls{AM} 0 
...
involves the participation of phonons -- quasi particles describing the 
...
in a meta stable state and eventually return back to the valence band, 
...
absorption process in Fig.~\ref{slme:fig-working}: 
...
\centering 
\captionsetup{width=0.9\textwidth}
\includegraphics[width=0.8\textwidth]{\figurepath/slme/Working.png} 
\caption{\label{slme:fig-working} Step by step visualization of the different 
...
Fig.~\ref{slme:fig-IV_char}b demonstrates the definition of the 
...
and loses less energy to thermal relaxation of electrons in the conduction 
...
\end{equation} 
The short-circuit current density $J_{sc}$, also known as the photo-generated 
...
original paper, Shockley and Queisser used a black-body spectrum of $T_s = 
...
Fig.~\ref{slme:fig-effchart}. Since its conception, the \gls{SLME} has been 
...
Fig.~\ref{slme:fig-SLMEcalc}. First, the absorptivity is computed from the 
...
\caption{Schematic representation of the calculation of the \gls{SLME} metric.} 
...
the \gls{SLME} and the section concludes with a discussion the obtained efficiencies of specific compounds.  
...
\caption{Calculated lattice parameters of the \mbox{CuAu-like} (\gls{CA}) and chalcopyrite (\gls{CH}) 
...
\multirow{2}{*}{Material}    & \multicolumn{3}{c}{\gls{CA}}  & \multicolumn{3}{c}{\gls{CH}} & \multicolumn{3}{c}{\gls{CH} (Ref~\cite{Hahn1953})} 
...
replacing the absorptivity by a parameterized sigmoid function, and analyzes for 
...
\resultsubsection{Logistic function model \label{slme:sec-logistic}}{https://github.com/mbercx/phd-thesis/blob/master/jupyter/slme/README.md\#logistic-function-model}{} 
...
\captionsetup{width=0.9\textwidth}
\centering 
\input{\figurepath/slme/logistic}
...
Fig.~\ref{slme:fig-Si_slme}a shows the experimental\footnote{We choose to 
...
\centering 
\captionsetup{width=0.9\textwidth}
\input{\figurepath/slme/si_slme.tex}
\caption{(\textbf{a}) Experimental absorption coefficient at $T = 300~\si{\kelvin}$ of 
crystalline silicon based on data taken from~\cite{green2008}. (\textbf{b}) Thickness dependence of the \gls{SLME} of silicon, based on the experimental spectrum in (\textbf{a}) and fraction of recombination $f_r 10^{-3}$.} 
\label{slme:fig-Si_slme} 
\end{figure} 
...
more reasonable value for silicon~\cite{Shockley1952,Trupke2003,Richter2012} -- the results shown in Fig.~\ref{slme:fig-Si_slme}b are obtained. 
